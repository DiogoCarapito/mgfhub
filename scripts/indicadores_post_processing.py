...
df['indexing'] = [
        str(row.id) + s # id
        + row.nome_abreviado + s # nome abreviado
        + row.designacao+s+str(row.area) + s # designacao
        + str(row.subarea) + s # subarea
        + str(row.dimensao) + s # dimensao
        + row.tipo_de_indicador + s # tipo de indicador
