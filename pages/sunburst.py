...
graphs = html.Div(
    [
        dbc.Row(
            [
                dbc.Col([], width=1),
                dbc.Col([dcc.Graph(id="sunburst_indicadores")], width=10),
                dbc.Col([], width=1),
            ]
        ),
    ]
...
def sunburst_update(dropdown_options_type, store_data):
    if store_data != []:
        df = pd.DataFrame(store_data)

        df = df[df["id_medico"] == dropdown_options_type]
    ...
    try:
        if dropdown_options_type == "unidade":
